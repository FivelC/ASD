...
public:
    ...
    void reserve(size_t);
    void resize(size_t);
    void resize(size_t, const T&);//new
    // Operators overload
...
template<class T> void TVector<T>::resize(size_t new_size,const T& value) { //new function
    /*if (new_size < 0) {
        throw std::invalid_argument("TVector.resize: Invalid argument 'new_size' - must be >= 0");
    }*/
    if (new_size < size()) {
        ...
        if (new_size >= _capacity) reserve(new_size + CAPACITY);
        for (size_t i = _size; i < new_size; i++) { _data[i] = value; _states[i] = TVectorElemState::busy };
    }
};
